/*************************************************************************
 *** FORTE Library Element
 ***
 *** This file was generated using the 4DIAC FORTE Export Filter V1.0.x NG!
 ***
 *** Name: FB_CondPWM
 *** Description: Simple FB with one algorithm
 *** Version:
***     1.0: 2025-10-03/sergdeb -  - 
 *************************************************************************/

#ifndef _FB_CONDPWM_H_
#define _FB_CONDPWM_H_

#include "simplefb.h"
#include "forte_bool.h"
#include "forte_int.h"
#include "forte_real.h"
#include "forte_uint.h"
#include "forte_array_at.h"


class FORTE_FB_CondPWM: public CSimpleFB {
  DECLARE_FIRMWARE_FB(FORTE_FB_CondPWM)

private:
  static const CStringDictionary::TStringId scm_anDataInputNames[];
  static const CStringDictionary::TStringId scm_anDataInputTypeIds[];
  
  static const CStringDictionary::TStringId scm_anDataOutputNames[];
  static const CStringDictionary::TStringId scm_anDataOutputTypeIds[];
  
  static const TEventID scm_nEventREQID = 0;
  
   static const TDataIOID scm_anEIWith[];
  static const TForteInt16 scm_anEIWithIndexes[];
  static const CStringDictionary::TStringId scm_anEventInputNames[];
  
  static const TEventID scm_nEventCNFID = 0;
  
   static const TDataIOID scm_anEOWith[]; 
  static const TForteInt16 scm_anEOWithIndexes[];
  static const CStringDictionary::TStringId scm_anEventOutputNames[];
  

  static const SFBInterfaceSpec scm_stFBInterfaceSpec;
static const CStringDictionary::TStringId scm_anInternalsNames[];
static const CStringDictionary::TStringId scm_anInternalsTypeIds[];
static const SInternalVarsInformation scm_stInternalVars;

virtual void setInitialValues();
          CIEC_BOOL &st_xStart() {
            return *static_cast<CIEC_BOOL*>(getDI(0));
          }
          
          CIEC_REAL &st_rY() {
            return *static_cast<CIEC_REAL*>(getDI(1));
          }
          
          CIEC_UINT &st_SP_Tp() {
            return *static_cast<CIEC_UINT*>(getDI(2));
          }
          
  CIEC_REAL &st_rYout() {
    return *static_cast<CIEC_REAL*>(getDO(0));
  }
  
  CIEC_UINT &st_iTimeYout() {
    return *static_cast<CIEC_UINT*>(getDO(1));
  }
  
  CIEC_BOOL &st_xYnol() {
    return *static_cast<CIEC_BOOL*>(getDO(2));
  }
  
  CIEC_BOOL &st_xFull() {
    return *static_cast<CIEC_BOOL*>(getDO(3));
  }
  
  CIEC_UINT &st_STAGE() {
    return *static_cast<CIEC_UINT*>(getDO(4));
  }
  
  CIEC_UINT &st_Stage() {
    return *static_cast<CIEC_UINT*>(getVarInternal(0));
  }
  
  CIEC_UINT &st_iY() {
    return *static_cast<CIEC_UINT*>(getVarInternal(1));
  }
  
  CIEC_REAL &st_rTimeYout() {
    return *static_cast<CIEC_REAL*>(getVarInternal(2));
  }
  
  CIEC_INT &st_iTimeYout1() {
    return *static_cast<CIEC_INT*>(getVarInternal(3));
  }
  
  CIEC_REAL &st_rSP_Tp() {
    return *static_cast<CIEC_REAL*>(getVarInternal(4));
  }
  

  void alg_REQ(void);

  FORTE_BASIC_FB_DATA_ARRAY(1, 3, 5, 5, 0);

public:
  FORTE_FB_CondPWM(CStringDictionary::TStringId pa_nInstanceNameId, CResource *pa_poSrcRes) :
       CSimpleFB(pa_poSrcRes, &scm_stFBInterfaceSpec, pa_nInstanceNameId, &scm_stInternalVars, m_anFBConnData, m_anFBVarsData) {
  };

  virtual ~FORTE_FB_CondPWM() = default;
};

#endif // _FB_CONDPWM_H_


