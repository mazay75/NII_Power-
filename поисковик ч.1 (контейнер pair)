...
int MatchDocument(const pair<int, vector<string>>& content, //будет возвращать релевантность документа
                  const set<string>& query_words) {
    set<string> query_words3;
    for (const string& word : content.second) {
        if (query_words.count(word) != 0) {query_words3.insert(word); }
    }
    return query_words3.size();
}
